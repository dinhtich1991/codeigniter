...
class My_frontend_auth {
	...
	public function check(){
		if(isset($_COOKIE[TT_PREFIX.'_user_logged']) && !empty($_COOKIE[TT_PREFIX.'_user_logged'])){
			...
			$user = $this->CI->db->select('id, sex, image, birth, username, password, salt, email, fullname, groupid')->where(array('username' => $cookie['username']))->from('user')->get()->row_array();
			//$group = explode("\n",$group['group']);
			...
			$auth = $this->CI->session->userdata('user');
			if(!isset($auth) || empty($auth)) return NULL;
			$auth = json_decode($auth, TRUE);
			$user = $this->CI->db->select('id, sex, image, birth, username, password, salt, email, fullname, groupid')->where(array('username' => $auth['username']))->from('user')->get()->row_array();
